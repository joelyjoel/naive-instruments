...
public:
  Ruler(NCursesWindow &canvas) : canvas(canvas) {}

private:
  void drawDivisions(float division) {
    for (float time = ceil(from / division) * division; time < to;
         time += division) {
...
private:
  ...
  string toLabelString(float time) { return std::to_string((int)time); }

  int xAtTime(float t) { return (t - from) / duration() * canvas.width; }
  double duration() { return to - from; }
