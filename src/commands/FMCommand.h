...
public:
    void run()
    {
        ...
        // Create the oscs
        std::vector<std::shared_ptr<Osc>>                 oscs;
        std::vector<std::shared_ptr<FrameStream<double>>> outputs;
        ...
        std::shared_ptr<FrameStream<double>> fundamentalPitch     = SignalString::parse( args[0] );
        std::shared_ptr<PitchConverter>      fundamentalFrequency = pitchToFrequency( fundamentalPitch );
        for ( int i = 0; i < numberOfOscs; ++i )
        {
            std::shared_ptr<Osc> osc = oscs[i];
            ...
            {
                std::shared_ptr<FrameStream<double>> ratio = SignalString::parse( args[i] );
        ...
        for ( int carrierIndex = 0; carrierIndex < numberOfOscs; ++carrierIndex )
        {
            for ( int modulatorIndex = 0; modulatorIndex < numberOfOscs; ++modulatorIndex )
            {
                ...
                if ( args.exists( key ) )
                {
                    std::shared_ptr<FrameStream<double>> modulation = args.signal( key );

                    std::shared_ptr<IntervalToRatio> ratio = intervalToRatio( outputs[modulatorIndex] * modulation );
        ...
        std::shared_ptr<FrameStream<double>> mixdown;
        for ( int i = 0; i < numberOfOscs; ++i )
        {
            std::shared_ptr<FrameStream<double>> oscOutput = outputs[i];
            const std::string                    key       = std::to_string( i ) + "-out";
            if ( args.exists( key ) || i == 0 )
            {
                std::shared_ptr<FrameStream<double>> level = args.signal( key, i == 0 ? "1" : "0" );
