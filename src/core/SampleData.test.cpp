...
TEST_CASE("Create a sample from a vector and find the peak") {
  REQUIRE(SampleData<int>({1, 2, 3, 4}).peak() == 4);
}

TEST_CASE("Root mean square of a sample") {
  REQUIRE(SampleData<int>({1, 1, 1, 1}).rms() == 1);
}

TEST_CASE("Overwrite a region of one sample using another") {
  SampleData<int> master(8);
  SampleData<int> overdub({1, 2, 3, 4});
...
TEST_CASE("Concatenating two samples") {
  SampleData<int> first({1, 2, 3, 4});
  SampleData<int> second({5, 6, 7, 8});

  auto combined = SampleData<int>::concat(first, second);
  REQUIRE(combined->read(7) == 8);
  REQUIRE(combined->read(0) == 1);
}

TEST_CASE("Mixing two samples together") {
  SampleData<int> master({1, 1, 1, 1, 1});
  SampleData<int> overdub({0, 1, 2, 3, 4});
...
TEST_CASE("Slicing a sample") {
  SampleData<int> master({0, 1, 2, 3, 4, 5, 6, 7, 8}, 1);
...
TEST_CASE(
  ...
  SampleData<float> sample({0, 1, 2, 3});
...
TEST_CASE("Selecting a single channel from a multichannel sample") {
  SampleData<int> stereo({0, 1, 2, 3, 4, 5, 6}, 2);
