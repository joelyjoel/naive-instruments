...
public:
  SampleData(int numberOfFrames, int numberOfChannels = 1)
      : numberOfChannels(numberOfChannels), numberOfFrames(numberOfFrames) {
    data = new T[numberOfChannels * numberOfFrames];
  }

  SampleData(std::vector<T> _data, int numberOfChannels = 1)
      : SampleData(_data.size(), numberOfChannels) {
...
public:
   ...
   */
  void write(SampleData<T> &y, int offset = 0) {
  ...
  void mix(SampleData<T> &y, int offset = 0) {
    for (int c = 0; c < y.numberOfChannels; ++c)
      for (int readFrame = 0; readFrame < y.numberOfFrames; ++readFrame) {
        int writeFrame = readFrame + offset;
        if (writeFrame >= 0 && writeFrame < numberOfFrames)
  ...
  std::shared_ptr<SampleData<T>> slice(int frame0 = 0, int frame1 = 0) {
    if (frame1 == 0)
      frame1 = numberOfFrames;
    ...
    auto sample =
        std::make_shared<SampleData<T>>(lengthOfNewSample, numberOfChannels);
  ...
  std::shared_ptr<SampleData<T>>
  selectChannels(std::vector<int> &channelsToSelect) {
    auto newSample = std::make_shared<SampleData<T>>(numberOfFrames,
                                                     channelsToSelect.size());
  ...
  std::shared_ptr<SampleData<T>> stereoFlip() {
  ...
  std::shared_ptr<SampleData<T>> selectChannel(int channel) {
    std::vector<int> channels = {channel};
    return selectChannels(channels);
  }

  static std::shared_ptr<SampleData<T>> concat(SampleData<T> &a,
                                               SampleData<T> &b) {
    int numberOfChannels = std::max(a.numberOfChannels, b.numberOfChannels);
    int numberOfFrames = a.numberOfFrames + b.numberOfFrames;
    std::shared_ptr<SampleData<T>> sample =
        std::make_shared<SampleData<T>>(numberOfFrames, numberOfChannels);
