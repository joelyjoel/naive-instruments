...
private:
    void action() override
    {
        ...
        {
            bufferMore();
            output[0] = internalBuffer[int( playhead ) % bufferSize];
...
public:
    ...
    float timeElapsed()
    {
        return playhead / sampleRate;
    }

    std::string describeTimeElapsed()
    {
        return std::to_string( int( timeElapsed() / 60 ) ) + ":" + std::to_string( fmod( timeElapsed(), 60 ) );
    }
};
