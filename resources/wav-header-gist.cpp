...
int main() {
  ...
  uint32_t inputFileSize = in.tellg();
  in.seekg(0, std::ios::end);
  inputFileSize = (uint32_t)in.tellg() - inputFileSize;
  in.seekg(0, std::ios::beg);

  printf("file size: %u\n", inputFileSize);

  wav_hdr wav;
  wav.ChunkSize = inputFileSize + sizeof(wav_hdr) - 8;
  wav.Subchunk2Size = inputFileSize;
